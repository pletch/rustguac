...
async fn guacd_to_ws(
    ...
    loop {
        ...
        // Overpaint findings are logged here rather than counted silently:
        // they describe a fault that shows itself once in days and cannot be
        // reproduced on demand, so the evidence has to already be in the
        // journal when a user reports the screen went black. See
        // FrameStats::observe_to_browser.
        if let Some(lines) = frame_stats.observe_to_browser(&text) {
            for line in lines {
                tracing::warn!(
                    session_id = %session_id,
                    "H.264 layer overpainted by guacd: {}", line
                );
            }
        }
